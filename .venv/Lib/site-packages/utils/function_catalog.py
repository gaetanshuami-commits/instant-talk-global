...
class FunctionCatalog:
    ...
    def refresh(self) -> None:
        ...
        try:
            # Force re-scan so that dynamically generated tool modules
            # (via chatdev.register_tool) are picked up immediately.
            manager.reload_functions()
...
def get_function_catalog(functions_dir: str | Path | None = None) -> FunctionCatalog:
    # If caller doesn't provide a directory, pick up current env override.
    if functions_dir is None:
        override = os.environ.get("MAC_FUNCTIONS_DIR")
        if override and override.strip():
            directory = Path(override).expanduser().resolve()
        else:
            directory = Path(FUNCTION_CALLING_DIR).resolve()
    else:
