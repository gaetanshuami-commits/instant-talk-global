...
_PACKAGED_SKILLS_ROOT = (REPO_ROOT / ".agents" / "skills").resolve()
...
def _resolve_skills_root() -> Path:
    ...
    cwd_root = (Path.cwd() / ".agents" / "skills").resolve()
    if cwd_root.exists() and cwd_root.is_dir():
        return cwd_root
    return _PACKAGED_SKILLS_ROOT


DEFAULT_SKILLS_ROOT = _resolve_skills_root()
