...
export default function PricingCards({ currentPlan = null }: PricingCardsProps) {
  ...
  async function handleCheckout(plan: string) {
    try {
      ...
      if (!res.ok || !data?.url) {
        alert("Erreur Stripe : " + (data?.error || "Impossible de créer la session de paiement"));
        setLoading(null);
        return;
      }
  ...
  return (
    <div className="grid gap-8 md:grid-cols-2">
      {PLANS.map((plan) => {
        const isCurrent = currentPlan === plan.key || (currentPlan === "trial" && plan.key === "premium");
