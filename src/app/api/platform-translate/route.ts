...
const LANG_NAMES: Record<string, string> = {
  ...
  sw: "Swahili",   ro: "Romanian",  el: "Greek",     sv: "Swedish",
  hu: "Hungarian", cs: "Czech",     th: "Thai",      vi: "Vietnamese",
...
// ── DeepL — primary for platform (high quality, fast for EU languages) ─────────
...
// ── Gemini — fallback when DeepL is unavailable ────────────────────────────────
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Languages not supported by DeepL → Gemini directly
    ...
    // DeepL first (fast, high-quality) — Gemini fallback if DeepL is down
    try {
    ...
    console.error("[platform-translate]", err)
